...
- `backend/app/features/span_ingestion/ingestion_client.py` - WAL data
- `backend/app/db_duckdb/datafusion_query.py` - Parquet data
...
def _convert_span_data_to_api_format(span_data):
    ...
    return {
        ...
        # ... core fields ...
        "attributes_json": attributes,  # Pass through unchanged
        "events_json": events,
...
```python
# REMOVE the same extraction block (lines ~180-222):
def _convert_parquet_row_to_api_format(row):
    # DELETE: junjo field extraction
    # Just pass attributes through
    return {
        # ... core fields ...
...
| `backend/app/db_duckdb/models.py` | Remove `junjo_*` fields from `SpanResponse` model |
...
| `frontend/src/features/traces/schemas/schemas.ts` | Remove `junjo_*` fields from `OtelSpan` type |
...
| Other components using `junjo_*` fields | Update to use SpanAccessor |

### Tests

| File | Change |
|------|--------|
| `backend/tests/test_v4_wal_fusion_integration.py` | Remove `junjo_*` fields from test helpers |
| `backend/tests/test_datafusion_query.py` | Update extraction tests |
| Frontend tests | Add SpanAccessor tests |

## What Stays Indexed

The DuckDB metadata table still indexes `junjo_span_type` for query filtering:
...
This is extracted during **Parquet indexing** (not API response formatting), which is a separate concern and should remain.

## Migration Path

1. **Create SpanAccessor** in frontend
2. **Update frontend components** to use SpanAccessor (can be done incrementally)
3. **Update frontend types** to remove `junjo_*` top-level fields
