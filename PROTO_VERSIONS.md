...
Protocol Buffer tools use different version formats that can appear confusing when comparing documentation to generated code. Here's what's normal and expected:
...
- **Full semantic version**: `v6.30.2` (shown in generated file headers)
  - Major version: `6` (breaking changes across protobuf ecosystem)
  - Minor version: `30` (feature releases)
  - Patch version: `2` (bug fixes)

**Both refer to the same version** - protoc uses semantic versioning internally (major.minor.patch), but releases are often referred to by just minor.patch for convenience.
...
**Generated Go files (header comment):**
```go
// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
//   protoc-gen-go v1.36.10
//   protoc        v6.30.2    ← Full semantic version (correct!)
// source: auth.proto
```
...
✅ **Go files showing `protoc v6.30.2` is CORRECT** - this is the same as v30.2, just using full semantic versioning
...
✅ **Python runtime version can differ from protoc version** - grpcio-tools 1.76.0 bundles protoc v30.2 (compiler) but generates code for protobuf 6.31.1 (runtime). This is normal and expected.

❌ **If Go files show anything other than `v6.30.2`** (like `v5.x.x` or `v7.x.x`), your protoc version is wrong - reinstall v30.2

❌ **If `protoc --version` shows anything other than `30.2`** (like `29.x`, `31.x`), you need to install the correct version

### How to Verify Your Installation is Correct

After installing, check:

**1. CLI shows correct compiler version:**
```bash
$ protoc --version
libprotoc 30.2  # ✓ Correct
```

**2. Regenerate a proto file and check the header:**
```bash
cd ingestion && make proto
head -10 proto_gen/auth.pb.go
# Should show: "protoc        v6.30.2"  ← This is correct!
```

**3. Check Python proto header:**
```bash
cd backend && ./scripts/generate_proto.sh
head -10 app/proto_gen/auth_pb2.py
# Should show: "Protobuf Python Version: 6.31.1"  ← This is correct!
```

If all three match these examples, your installation is correct and will generate identical code to CI/CD.
...
- **GitHub Actions** (`.github/workflows/proto-staleness-check.yml`): Downloads protoc v30.2 binary
